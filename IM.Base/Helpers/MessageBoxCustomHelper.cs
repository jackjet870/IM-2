...
namespace IM.Base.Helpers
{
  ...
  /// MessageBox.Show("hola", "hola", MessageBoxButtons.AbortRetryIgnore);
  /// MsgBoxUtil.UnHackMessageBox();
  /// </example>
  /// <history>[ECANUL] 04/04/2016 CREATED </history>
  public class MessageBoxCustomHelper
  {
    #region Atributos
    ...
    /// <history>[ECANUL] 04-04-2016 Created</history>
    public static void CustomMeesageBox(params string[] textoBotones)
    {
      //guardar referencia global a la lista
      MessageBoxCustomHelper.textoBotones = textoBotones;
      // Si hay al menos una forma...se debe trabajar como aplicación forms
      if (Application.Current.Windows.Count > 0)
      {
        //Se llama el delegado desde la clase hijo del actual Windows
        Application.Current.Windows[0].Dispatcher.BeginInvoke(new BuscarMsgBoxDelegate(BuscaMessageBox));
      }
      else //si no no se ha creado una instrancia de msgbox y se espera a que se cree
        //llamado asincrono 
        ThreadPool.QueueUserWorkItem(EsperarYCambiarMsgBoxWC);
    }

    /// <summary>
    /// Destruye la referencia al nuevo Message box
    /// </summary>
    /// <history>[ECANUL] 04-04-2016 Created</history>
    public static void ReturnToCommonMessageBox()
    {
      DestroyWindow(MessageBoxCustomHelper.globalHandler);
    }
    ...
    /// de valores iniciales al llamr a CustomMeesageBox
    /// </summary>
    ...
    private static bool ProcesaMessageBoxEnForms(IntPtr handler, IntPtr longPointer)
    {
      ...
      else
      {
        ...
        MessageBoxCustomHelper.globalHandler = handler;
        return false;
      }
    }
    ...
    /// <summary>
    /// Wrapper para simplificar el llamado a EnumThreadWindows desde HackMessageBox
    /// </summary>
    /// <history>[ECANUL] 04-04-2016 Created</history>
    ...
    /// </summary>
    /// <param name="stateInfo">no se usa, pero es neceario para crear un waitcallback y asi realizar ejecución desde el threadpool</param>
