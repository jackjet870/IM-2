...
namespace IM.Base.Forms
{
  ...
  { 
    ...
    private EnumInvitationType2 _invitationType;
    ...
    public frmInvitationBase(IM.Model.Enums.EnumInvitationType2 invitationType, UserData userData, int guestID, EnumInvitationMode invitationMode, bool allowReschedule = true)
    {
    ...
    private void btnChange_Click(object sender, RoutedEventArgs e)
    {
      ...
      if (allowReschedule)//si se permite hacer reschedules
      {
        ...
        if (invitD.Date == _serverDateTime.Date || txtBookingDate.SelectedDate.Value.Date > _serverDateTime.Date)
        {
          ...
          if (_user.HasPermission(_invitationType == EnumInvitationType2.Host ? EnumPermission.HostInvitations : EnumPermission.PRInvitations, EnumPermisionLevel.Special)
              || _user.HasPermission(_invitationType == EnumInvitationType2.Host ? EnumPermission.HostInvitations : EnumPermission.PRInvitations, EnumPermisionLevel.SuperSpecial))
          {
            //PR Contract
    ...
    private void btnRebook_Click(object sender, RoutedEventArgs e)
    {
      ...
      if (_invitationType == EnumInvitationType2.InHouse || _invitationType == EnumInvitationType2.Animation || _invitationType == EnumInvitationType2.Regen)
      {
        //No directa
    ...
    private void  imgDetails_MouseLeftButtonDown(object sender, System.Windows.Input.MouseButtonEventArgs e)
    {
      ...
      var frmGuest = new frmGuest(_user, item.guID, true, _invitationType == EnumInvitationType2.InHouse, false);
      frmGuest.Owner = this;
      frmGuest.ShowDialog();
    ...
    private void ControlsConfiguration()
    {
      ...
      {
        case EnumInvitationType2.InHouse:
          InHouseControlsConfig();
          break;
        case EnumInvitationType2.OutHouse:
          OutHouseControlsConfig();
          break;
        case EnumInvitationType2.Host:
          HostControlsConfig();
          break;
        case EnumInvitationType2.Animation:
          AnimationControlsConfig();
          break;
        case EnumInvitationType2.Regen:
          RegenControlsConfig();
          break;
        case EnumInvitationType2.External:
          InHouseControlsConfig();
    ...
    private void EnableControls()
    {
      ...
      if (enable)
      {
        ...
        if (_invitationType == EnumInvitationType2.OutHouse && _invitationMode == EnumInvitationMode.modAdd)
        {
          txtOutInvitation.IsEnabled = true;
        }
        else if (_invitationMode == EnumInvitationMode.modEdit && _invitationType == EnumInvitationType2.InHouse) //si es una invitacion inhouse
        {
        ...
        else if (_invitationMode == EnumInvitationMode.modEdit && _invitationType == EnumInvitationType2.InHouse)
        {
          txtOutInvitation.IsEnabled = true;
        }
      }
      ...
      if (_invitationType != EnumInvitationType2.Regen && _invitationType != EnumInvitationType2.Animation)
      {
      ...
      {
        if (_invitationType != EnumInvitationType2.Host) // si no es el modulo host
        {
        ...
        if (_invitationType != EnumInvitationType2.OutHouse)
        {
          ...
          if (txtDeparture.SelectedDate.HasValue && (txtDeparture.SelectedDate.Value.Date >= _serverDateTime.Date)
              && ((_isNewInvitation || dtInvit.Date == _serverDateTime.Date)
                  || (_user.HasPermission(_invitationType == EnumInvitationType2.Host ? EnumPermission.Host : EnumPermission.PRInvitations, EnumPermisionLevel.Special)
      ...
      if (_invitationMode == EnumInvitationMode.modEdit)
      {
        ...
        if (_invitationType != EnumInvitationType2.OutHouse && _invitationType != EnumInvitationType2.Host)
        {
      ...
      if (_invitationMode == EnumInvitationMode.modEdit)//si se esta modificando
      {
        ...
        if (_invitationType != EnumInvitationType2.OutHouse)//si no es el modulo outside
        {
          //solo se permite modificar la fecha de salida si tiene permiso especial de invitaciones
          if (!_user.HasPermission(_invitationType == EnumInvitationType2.Host ? EnumPermission.Host : EnumPermission.PRInvitations, EnumPermisionLevel.Special))
          {
      ...
      if (_invitationMode == EnumInvitationMode.modEdit)//si se esta modificando
      {
        if (_invitationType != EnumInvitationType2.OutHouse && _invitationType != EnumInvitationType2.Host)//si no es el modulo outside ni el de Host
        {
      ...
      if (_invitationType == EnumInvitationType2.InHouse || _invitationType == EnumInvitationType2.Host)
      {
      ...
      if (_invitationType == EnumInvitationType2.InHouse)
      {
    ...
    private void SetupChangeRescheduleRebook()
    {
        ...
        || _user.HasPermission(_invitationType == EnumInvitationType2.Host ? EnumPermission.HostInvitations : EnumPermission.PRInvitations, EnumPermisionLevel.SuperSpecial))
      {
        ...
        else if (chkShow.IsChecked.HasValue && chkShow.IsChecked.Value) // si tiene show
        {
          ...
          if (_invitationType != EnumInvitationType2.Animation || _invitationType != EnumInvitationType2.Regen)
          {
        ...
        // si la fecha de invitacion es antes de hoy y no es un reschedule y su fecha de booking es despues de hoy y tiene permiso estandar de invitaciones
                  ...
                  && _user.HasPermission(_invitationType == EnumInvitationType2.Host ? EnumPermission.HostInvitations : EnumPermission.PRInvitations, EnumPermisionLevel.Standard)
                )
        {
          // no se permite reschedule
          EnableButtonsChangeRescheduleRebook(true, false, true);
    ...
    private void SetInitialValues()
    {
      ...
      //Location
      if (_invitationType != EnumInvitationType2.Host)//si no es modulo Host
      {
    ...
    private bool GeneralValidation()
    {
      ...
      if (_invitationType == EnumInvitationType2.InHouse && !ValidateFolioInHouse()) //validamos que el folio de invitación inhouse
      {
        Helpers.UIHelper.ShowMessage("The reservation number has been assigned previously");
        res = false;
      }
      else if (_invitationType == EnumInvitationType2.OutHouse && String.IsNullOrEmpty(txtPRContract.Text))//validamos que haya seleccionado un proveedor Contact
      {
      ...
      else if (_invitationType != EnumInvitationType2.Host && String.IsNullOrEmpty(txtSalesRoom.Text))//validamos que haya seleccionado una sala de ventas
      {
      ...
      else if (_invitationType == EnumInvitationType2.Host && String.IsNullOrEmpty(txtLocation.Text))  // validamos la locacion
      {
        Helpers.UIHelper.ShowMessage("Specify a Location");
        cmbLocation.Focus();
        res = false;
      }
      else if (_invitationType == EnumInvitationType2.OutHouse)
      {
