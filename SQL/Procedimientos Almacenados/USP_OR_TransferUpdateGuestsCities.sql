if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[USP_OR_TransferUpdateGuestsCities]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[USP_OR_TransferUpdateGuestsCities]
GO
...
**
*/
create procedure [dbo].[USP_OR_TransferUpdateGuestsCities]
as
set nocount on
...
where (G.guCity is null or G.guCity <> T.tCity) and T.tCity <> ''
